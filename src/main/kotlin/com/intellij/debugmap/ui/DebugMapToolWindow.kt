            ...
            for (group in groups) {
                addNode(
                    data = DebugMapNode.Group(
                        group.id, group.name, group.id == activeGroupId,
            ...
            IconActionButton(
                ...
                onClick = {
                    ...
                    val name = Messages.showInputDialog(
                        project,
                        DebugMapBundle.message("dialog.new.group.label"),
                        DebugMapBundle.message("dialog.new.group.title"),
                        null,
                        defaultName,
                        null
                    )
    ...
    when (kind) {
        ...
        SelectionKind.BOOKMARKS -> WriteAction.run<Exception> {
            nodes.filterIsInstance<DebugMapNode.BookmarkItem>()
                .forEach { service.removeBookmarkFromGroup(it.def.groupId, it.def.fileUrl, it.def.line) }
        }

        SelectionKind.BREAKPOINTS -> WriteAction.run<Exception> {
            nodes.filterIsInstance<DebugMapNode.BreakpointItem>()
                .forEach { service.removeBreakpointFromGroup(it.def.groupId, it.def.fileUrl, it.def.line) }
        }
